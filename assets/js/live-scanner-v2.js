...
(function (global) {
  ...
  async function fetchLive() {
    ...
    try {
      // Reuse the existing dashboard refresh path so the table itself is re-rendered
      // from the live gateway instead of only updating the status badge.
      if (refreshEl && !refreshEl.disabled) {
        global._forceLiveSpreadRefresh = true;
        const trackingName = refreshEl.getAttribute('data-track');
        if (trackingName) refreshEl.removeAttribute('data-track');
        refreshEl.click();
        if (trackingName) refreshEl.setAttribute('data-track', trackingName);
      }

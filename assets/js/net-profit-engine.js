...
var CoinNavigatorNetProfit = (() => {
  ...
  // src/net-profit/index.ts
  var net_profit_exports = {};
  __export(net_profit_exports, {
    EXCHANGE_FEE_ESTIMATES: () => EXCHANGE_FEE_ESTIMATES,
    NET_PROFIT_THRESHOLDS: () => NET_PROFIT_THRESHOLDS,
    SUPPORTED_EXCHANGES: () => SUPPORTED_EXCHANGES,
    bucketAmountUsd: () => bucketAmountUsd,
    bucketNetProfitUsd: () => bucketNetProfitUsd,
    bucketSpreadPct: () => bucketSpreadPct,
    calculateNetProfit: () => calculateNetProfit,
    formatPct: () => formatPct,
    formatQty: () => formatQty,
    formatUsd: () => formatUsd,
    getEstimatedTakerFeePct: () => getEstimatedTakerFeePct,
    getExchangeFeeEstimate: () => getExchangeFeeEstimate,
    isNonNegativeNumber: () => isNonNegativeNumber,
    isPositiveNumber: () => isPositiveNumber,
    parseOptionalNumber: () => parseOptionalNumber,
    validateNetProfitInput: () => validateNetProfitInput,
  ...
  function roundMoney(value) {
    return toNumber(value.toDecimalPlaces(8, decimal_default.ROUND_HALF_UP));
  }
  ...
  function calculateNetProfit(input) {
    ...
    const networkAsset = d((_a = input.networkFeeAsset) != null ? _a : 0);
    const additionalUsd = d((_b = input.additionalCostUsd) != null ? _b : 0);
    ...
    const idealSellable = decimal_default.max(idealQty.minus(withdrawalAsset).minus(networkAsset), 0);
    const idealProceeds = idealSellable.mul(sellPrice);
    const slippedProceeds = sellableQty.mul(effectiveSellPrice);
    const estimatedSlippageCostUsd = decimal_default.max(idealProceeds.minus(slippedProceeds), 0);
  ...
  function bucketNetProfitUsd(net) {
  ...
  function formatUsd(value, signed = false) {
    ...
    let body;
    if (abs2 === 0) body = "0.00";
    else if (abs2 < 0.01) body = abs2.toFixed(6);
    else if (abs2 < 10) body = abs2.toFixed(4);
    else if (abs2 < 1e3) body = abs2.toFixed(2);
    else body = abs2.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
    const sign2 = signed ? value > 0 ? "+" : value < 0 ? "\u2212" : "" : value < 0 ? "\u2212" : "";
    return `${sign2}$${body}`;
  }
  function formatPct(value, signed = false) {
    if (!Number.isFinite(value)) return "\u2014";
    const abs2 = Math.abs(value);
    const body = abs2 < 1e-3 ? abs2.toFixed(6) : abs2.toFixed(4);
    const sign2 = signed ? value > 0 ? "+" : value < 0 ? "\u2212" : "" : value < 0 ? "\u2212" : "";
    return `${sign2}${body}%`;
  }
  function formatQty(value) {
    ...
    const abs2 = Math.abs(value);
    if (abs2 === 0) return "0";
    if (abs2 < 1e-4) return abs2.toExponential(2);
    if (abs2 < 1) return abs2.toFixed(6);
    if (abs2 < 1e3) return abs2.toFixed(4);
    return abs2.toLocaleString("en-US", { maximumFractionDigits: 4 });
  }
  ...
  return __toCommonJS(net_profit_exports);
})();
...
decimal.js/decimal.mjs:
   ...
   *)
*/
if (typeof window !== "undefined") { window.CoinNavigatorNetProfit = CoinNavigatorNetProfit; }
